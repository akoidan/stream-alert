...
int main(int argc, char **argv)
{
		...
		(mt.pbFormat != NULL))
	{
		...
		// If the source is YUY2, convert to RGB24 before writing
		std::vector<unsigned char> convertedBuffer;
		unsigned char *dataToWrite = reinterpret_cast<unsigned char*>(pBuffer);
		long dataSizeToWrite = buffer_size;
		if (pVih->bmiHeader.biCompression == MAKEFOURCC('Y','U','Y','2'))
		{
			int width = pVih->bmiHeader.biWidth;
			int height = std::abs(pVih->bmiHeader.biHeight);
			int pixels = width * height;
			convertedBuffer.resize(pixels * 3);
			const unsigned char *src = reinterpret_cast<unsigned char*>(pBuffer);
			unsigned char *dst = convertedBuffer.data();
			...
			for (int y = 0; y < height; ++y)
			{
				...
				for (int x = 0; x < width; x += 2)
				{
					...
					unsigned char *pixel0 = dstRow + x * 3;
					unsigned char *pixel1 = dstRow + (x + 1) * 3;
			...
			pVih->bmiHeader.biSizeImage = static_cast<DWORD>(convertedBuffer.size());
			buffer_size = static_cast<long>(convertedBuffer.size());
			dataToWrite = convertedBuffer.data();
			dataSizeToWrite = buffer_size;
		}
		...
		bfh.bfSize = sizeof(bfh) + dataSizeToWrite + cbBitmapInfoSize;
		...
		WriteFile(hf, dataToWrite, dataSizeToWrite, &dwWritten, NULL);
