...
const process__default = /* @__PURE__ */ _interopDefault(process$1);
const fs__default = /* @__PURE__ */ _interopDefault(fs);
const readline__default = /* @__PURE__ */ _interopDefault(readline);
const clc__default = /* @__PURE__ */ _interopDefault(clc);
const bindings__default = /* @__PURE__ */ _interopDefault(bindings);
...
let CustomLogger = class extends common.ConsoleLogger {
  logFormat(level, message, levelColor, messageStyle) {
    const timestamp = clc__default.default.xterm(100)(`[${this.getCurrentTime()}]`);
  ...
  log(message) {
    console.info(this.logFormat("INFO", message, clc__default.default.bold.blue, clc__default.default.cyan));
  }
  error(message, trace) {
    console.error(this.logFormat("ERROR", message?.message ?? message, clc__default.default.bold.redBright, clc__default.default.red));
    if (trace ?? message?.stack) {
      console.error(clc__default.default.red(trace ?? message?.stack));
    }
  }
  warn(message) {
    console.warn(this.logFormat("WARN", message, clc__default.default.bold.yellow, clc__default.default.yellow));
  }
  debug(message) {
    console.debug(this.logFormat("DEBUG", message, clc__default.default.xterm(2), clc__default.default.xterm(7)));
  }
  verbose(message) {
    console.log(this.logFormat("VERBOSE", message, clc__default.default.bold.magenta, clc__default.default.xterm(7)));
  }
};
...
let NativeModule = class {
  ...
  onModuleInit() {
    this.logger.log(`Loaded native library from ${clc__default.default.bold.green(this.native.path)}`);
  }
};
  ...
  common.Module({
    providers: [
      common.Logger,
      {
        provide: Native,
        useFactory: () => {
          return bindings__default.default("native");
        }
      }
    ],
    exports: [Native]
...
process__default.default.env["NODE_CONFIG_TS_DIR"] = path.join(__dirname, "config");
const configPath = path.join(process__default.default.env.NODE_CONFIG_TS_DIR, "default.json");
...
async function bootstrap() {
  ...
  try {
    ...
    fs__default.default.writeFileSync(configPath, updatedConfig);
